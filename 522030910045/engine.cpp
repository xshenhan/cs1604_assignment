...
// load the map from a stream
Field *loadMap(std::istream &is) {
    int m = -1, n = -1, nt = 0, nu = 0; // need to init, but don't know how to init (0 or -1)
    string commandline;
    getline(is, commandline);
    stringstream ss(commandline);
    string sm, sn, snt, snu;
//    ss >> sm >> sn >> snt >> snu;
//    if (!IsNumber(sm) || !IsNumber(sn) || !IsNumber(snt) || !IsNumber(snu)){
//        cout << "Failed to load map!" << endl; NOERROR = false;
//        return nullptr;
//    }
//    m = stoi(sm); n = stoi(sn); nt = stoi(snt); nu = stoi(snu);
    ...
    for (int i = 0; i < nt; i++) {
        ...
        switch (T) {
            case 'W':
                newMap->set_terrian(r, c, WATER);
                break;
            case 'M':
                newMap->set_terrian(r, c, MOUNTAIN);
                break;
            case 'F':
                newMap->set_terrian(r, c, FOREST);
                break;
            case 'A':
                newMap->set_terrian(r, c, A);
                break;
            default:
                cout << "Failed to load map!" << endl;
                NOERROR = false;
    ...
    for (int i = 0; i < nu; i++) {
        ...
        switch (u[0]) {
            case 'F':
                newMap->set_unit(r, c, FT, player);
                break;
            case 'K':
                newMap->set_unit(r, c, KN, player);
                break;
            case 'A':
                newMap->set_unit(r, c, AR, player);
                break;
            case 'M':
                newMap->set_unit(r, c, MG, player);
                break;
            default:
                cout << "Failed to load map!" << endl;
                NOERROR = false;
